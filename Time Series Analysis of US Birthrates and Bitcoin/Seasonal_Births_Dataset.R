...
#Syed's attempt at forecasting
best_model <- best.model  # Replace with your best model
future_forecast <- forecast(best_model, h = 5 * 12)
plot(future_forecast)

#SARIMA-GARCH model ended up being unneccessary
#SARIMA-GARCH model
#need to use seasonal_diff
seasonal_diff %>%
...
m11.births <- arima(seasonal_diff, order = c(1,0,2), seasonal = list(order=c(2,0,2), period=12))
m11.births  #Note: model 6 is better but trying this for first run of SARIMA-GARCH model

checkresiduals(m11.births)
Box.test(resid(m11.births), lag = 12, type = "Ljung-Box") #test seasonal autocorrelation

#archTest(m11.births, lag = 12) - NEED TO FIGURE OUT HOW TO USE
...
spec <- ugarchspec(
  ...
  mean.model = list(
    armaOrder = c(1, 2) #the parameters previously identified using auto.arima()
